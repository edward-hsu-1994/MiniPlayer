...
function Init(event?) {
    ...
    miniVideo.onmousedown = function (event) {
        if (event.offsetX <= 32 && event.offsetY <= 32) Moveable = true;
    }
    ...
    miniVideo.onclick = function (event) {
        if (event.offsetX <= 32 && event.offsetY <= 32) return;
        if (!video.paused) {
            video.pause();
        } else {
            video.play();
    ...
    function StartMiniPlayer() {
        ...
        timer = setInterval(function () {
            ...
            context.drawImage(<HTMLImageElement>video, 0, 0, miniVideo.width, miniVideo.height);
        }, 60);
    }
